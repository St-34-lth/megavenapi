...
class MegavenApi:
          ...
          def getInventoryLocation(filters):
                    ...
                    return response
          @staticmethod
          def setInventoryLocationStock(product,qty,action):
                    method = 'post'
                    endPoint ='InventoryLocationStock/ProductStockUpdate'
                    mvRecordAction = {"mvRecordAction":action}
                    mvProductStockUpdateList = {"mvProductStockUpdateList": [
                                                  {
                                                            "ProductID": product.getProductID(),
                                                            "ProductSKU": product.getProductSKU(),
                                                            "ProductQuantity": qty,
                                                            "InventoryLocationID": product.getProductLocationID()
                    ...
                    payload = dict(MegavenApi._apiKey,**mvProductStockUpdateList )
                    response = MegavenApi.fetch(method, payload, endPoint)
                    MegavenApi.saveResponse(response.text, 'inventoryLocations', 'json')
                    print(response.text)
          @staticmethod 
          ...
          def makeSalesOrder(sale,action):
                    ...
                    mvSalesOrder= {"mvSalesOrder": 
                              {  
                                        "SalesOrderClientId": int(sale.getSaleClientID()),
                                        'SalesOrderTypeId':3,
                                        ...
                                        "SalesOrderDetails": [
                                                            {
                                                                      ...
                                                                      "SalesOrderRowProductSKU": int(sale.getSaleProductSKU()),
                                                                      "SalesOrderRowProductDescription": sale.getSaleProductDescription(),
                                                                      "SalesOrderRowQuantity": sale.getSaleTotalQty(),
                                                                      'SalesOrderRowShippedQuantity': str(sale.getSaleTotalQty()),
                                                                      'SalesOrderRowInvoicedQuantity': str(sale.getSaleTotalQty()),
                                                                      "SalesOrderRowUnitPriceWithoutTaxOrDiscount": str(sale.getSaleNominalPrice()),
                                                                      #"SalesOrderTotalTaxAmount": str(sale.getSaleTaxAmount()),
                                                                      #"SalesOrderRowTotalDiscountAmount": str(sale.getSaleDiscountAmount()),
                                                                      #"SalesOrderRowTotalAmount": str(sale.getSaleFinalPrice()),
                                                                      "SalesOrderRowTaxID": sale.saleTaxID,
                                                                      "SalesOrderRowDiscountID": sale.saleDiscountID,
                                                            }
                                                           ],
                                        ...
                                        "SalesOrderTotalQuantity": str(sale.getSaleTotalQty()),
                                        "SalesOrderAmountSubtotalWithoutTaxAndDiscount": sale.getSaleNominalPrice(),
                                        "SalesOrderAmountTotalDiscount": str(sale.getSaleDiscountAmount()),
                                        "SalesOrderAmountTotalTax": str(sale.getSaleTaxAmount()),
                                        "SalesOrderAmountGrandTotal":str(sale.getSaleFinalPrice()),
                              },
                        "mvRecordAction": action}
                    payload = dict(MegavenApi._apiKey, **mvSalesOrder)
                    print(payload)
                    response = MegavenApi.fetch(method,payload,endPoint)
                    return response 